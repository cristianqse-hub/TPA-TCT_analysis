...
DEFAULT_APPLIED_VOLTAGE = 850.0

# Fixed Caughey-Thomas/Thomas mobility parameters. They are not fit parameters.
FIXED_MOBILITY_PARAMETERS = {
    "electron_mu0": 950.0231115124747,
    "electron_vsat": 19999775.80086712,
    "electron_beta": 1.1500965108667693,
    "hole_mu0": 119.99176109978578,
    "hole_vsat": 12009759.527642306,
    "hole_beta": 1.2502079889374755,
}
...
def ensure_manual_trapping_json(
    ...
    if path.exists():
        ...
        payload.setdefault("description", "Manual F2W1 lineal-field trapping-simulation parameters.")
    ...
    payload = {
        "description": (
            "Manual F2W1 lineal-field trapping-simulation parameters. Edit profile_index to choose "
...
def ensure_profile_fit_results_json(
    ...
    if path.exists():
        ...
        payload = {
            "description": (
                "Best stored lineal-field trapping-model fit results for each F2W1 profile index. "
    ...
    payload.setdefault("description", "Best stored lineal-field trapping-model fit results for each F2W1 profile index.")
...
def append_good_multistart_fit(
    ...
    if path.exists():
        ...
        payload = {
            "description": "Accepted multistart lineal-field trapping-model fits, stored in completion order.",
            "goodness_metric": "chi2_dof",
            "accepted_fits": [],
        }

    payload.setdefault("description", "Accepted multistart lineal-field trapping-model fits, stored in completion order.")
    payload.setdefault("goodness_metric", "chi2_dof")
...
def plot_manual_trapping_control(
    ...
    ax_velocity.set_ylabel("velocity [um/ns]")
    ...
    ax_field.set_ylabel("field [V/um]")
...
def profile_withTrapps(x_vec, fit_parameters, beam_parameters, stepsPerAC, n_z_grid=DEFAULT_N_Z_GRID):
    ...
    applied_voltage = float(beam_parameters.get("applied_voltage", DEFAULT_APPLIED_VOLTAGE))
    field_end = np.clip(float(fit_parameters["field_end"]), 1e-9, z2)
    field_slope_input = min(float(fit_parameters["field_slope"]), -1e-12)
    min_slope_for_nonnegative_end = -2.0 * applied_voltage / field_end**2
    field_slope = max(field_slope_input, min_slope_for_nonnegative_end + 1e-12)
    z_rel = activeVolume_dis - z0_shifted

    # Linear/trapezoidal electric field. The intercept at z0 is solved so that
    # integral E(z) dz over [z0, z0 + field_end] equals the applied voltage.
    # Very negative slopes are clipped to the triangular-field limit E(field_end)=0.
    field_intercept = (applied_voltage - 0.5 * field_slope * field_end**2) / field_end
    efield = np.zeros_like(activeVolume_dis, dtype=float)
    active_field = (z_rel >= 0.0) & (z_rel <= field_end)
    efield[active_field] = field_intercept + field_slope * z_rel[active_field]
    efield = np.maximum(efield, 1e-12)
    field_integral = trapz(efield[active_field], activeVolume_dis[active_field]) if np.any(active_field) else 0.0

    mobility = dict(FIXED_MOBILITY_PARAMETERS)
    mobility.update({key: float(fit_parameters[key]) for key in mobility if key in fit_parameters})
    efield_cm = efield * 1e4
    epsilon = 1e-10
    vdrift_mue = (
        mobility["electron_mu0"] * efield_cm
        / (1.0 + (mobility["electron_mu0"] * efield_cm / mobility["electron_vsat"])**mobility["electron_beta"])**(1.0 / mobility["electron_beta"])
    ) * 1e-5 + epsilon
    vdrift_muh = (
        mobility["hole_mu0"] * efield_cm
        / (1.0 + (mobility["hole_mu0"] * efield_cm / mobility["hole_vsat"])**mobility["hole_beta"])**(1.0 / mobility["hole_beta"])
    ) * 1e-5 + epsilon
    ...
    material_response = {
        ...
        "field_slope_input": field_slope_input,
        "min_slope_for_nonnegative_end": min_slope_for_nonnegative_end,
        "field_intercept": field_intercept,
        "field_integral": field_integral,
        "applied_voltage": applied_voltage,
        "mobility_parameters": mobility,
...
def fit_bounds(beam_parameters, names=FIT_PARAMETER_NAMES, lower_overrides=None, upper_overrides=None):
    upper = {
        "field_end": float(beam_parameters["z2"]),
        "field_slope": -1e-12,
        "intensity_scale": 3.0,
        "tau_e_ns": 1e4,
        "tau_h_ns": 1e4,
